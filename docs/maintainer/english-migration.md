...
2. What's the migration cutoff version? → Revised (as of v0.4.0 release, 2026-08-17): complete Phase 1 by v0.2.0 (DONE), Phase 2 by **v0.5.0**, Phase 3 by v0.6.0, Phase 4 CI-enforced from **v0.5.0**. Phase 1 compliance holds through v0.4.0 (no new Japanese shipped spec introduced). Phase 2 critical-path migration (harness-work / tdd-implement / parallel-worktree / worker / reviewer) is **not** complete in v0.4.0 and moves to v0.5.0.

   **Why the cutoff moved.** v0.4.0 exists to ship three guardrail fixes, not as a migration milestone. Without them PreToolUse `ask` / `deny` were emitted in an envelope Claude Code ignores for that event, so every deny-class rule was advisory rather than enforcing. Holding a correctness release behind an unrelated prose-language migration would have left consumers running guardrails that do not guard. Nothing about the migration changes in substance — same five files, same phases — only the version label. The previous revision (v0.3.0, 2026-04-23) had set Phase 2 at v0.4.0.
