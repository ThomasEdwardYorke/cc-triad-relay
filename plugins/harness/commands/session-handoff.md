...
陳腐化 signal 一覧 (S-01 〜 S-18、severity 付き):
...
| S-18 | **持ち越し item の構造的可視化** (universal、`feedback-loop` 機構): backlog 内の `session-(\d{4}-\d{2}-\d{2})-` 形式 archive 参照のうち今日から 7 日以上前の参照件数を集計。**3 件以上で WARN** (構造的 blocker の可能性、`count >= 3`)、**5 件以上で FAIL** (`count >= 5`、item-level 解消ではなく root-cause fix を要求)。同一 archive を複数 item が参照する場合は **重複 dedupe せず参照数で count** (item 単位の持ち越し圧迫の表現として) | regex 抽出 + ISO date 比較 (`new Date(today) - new Date(matched-date) >= 7 * 86400000`)、git unavailable でも動作 | WARN (3+) / FAIL (5+) |
