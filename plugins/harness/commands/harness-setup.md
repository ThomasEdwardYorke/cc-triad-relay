...
`harness init` (CLI) または `/harness-setup init` を実行すると、`template/`
ディレクトリの全 bootstrap files を `{{PROJECT_NAME}}` placeholder 置換付き
で project root に展開する。**idempotent** — 既存 file は touch しない
(consumer の手作業を上書きしない)。
...
`init` だけでは「動作する」状態にならない。以下を順に行う:

1. **編集**: `harness.config.json` を project に合わせる
   (`projectName` / `language` / `testCommand` / `protectedDirectories` /
   `worktree.defaultBaseBranch` / `release.strategy` など)。
   Schema 全項目は `plugins/harness/schemas/harness.config.schema.json` 参照。
2. **編集**: `CLAUDE.md` の `{{PROJECT_DESCRIPTION}}` を実内容で置換、
   stack / 設計方針を追記。
3. **編集**: `.coderabbit.yaml` の `language` / `tone_instructions` /
   `path_instructions` をプロジェクトに合わせる。
   **`reviews.request_changes_workflow: true` は触らない** (APPROVED 自動発火に必要)。
4. **External deps**: `gh` / `codex` / `coderabbit` の CLI を install
   (`harness check` で WARN が消えるまで)。詳細は `template/README.md` Step 4。
5. **CodeRabbit App**: GitHub repo に install (https://app.coderabbit.ai/、
   admin 権限必要)、`.coderabbit.yaml` を default branch に push して effective 化。
6. **`harness check`** を実行して全 OK / WARN を確認。

詳細 step-by-step は `template/README.md` 参照 (consumer 向け entry point)。
