...
// project-specific id schemes (`entry-alpha`, `phase-section.subitem`,
// `work-item-beta`, `2026.04.25`) all parse without configuration.
const HEADING_RE = /^###[ \t]+\[(Critical|High|Med|Low)\][ \t]+([A-Za-z0-9._-]+)[ \t]+(.+?)[ \t]*$/;
 ...
 */
function parseYamlBlockBody(lines) {
    const fields = {};
    let malformed = false;
 ...
 * than disappearing.
 */
function applyYamlOverrides(entry, yaml, filepath) {
    const next = { ...entry };
    if (typeof yaml.id === "string" && yaml.id.length > 0) {
        next.id = yaml.id;
    }
    if (typeof yaml.priority === "string" && yaml.priority.length > 0) {
        if (VALID_PRIORITIES.includes(yaml.priority)) {
            next.priority = yaml.priority;
        }
        else {
            process.stderr.write(`[harness backlog] ${filepath}:${entry.lineNumber} priority=${JSON.stringify(yaml.priority)} is not one of ${JSON.stringify(VALID_PRIORITIES)}; keeping heading default "${entry.priority}".\n`);
        }
    }
    if (typeof yaml.status === "string" && yaml.status.length > 0) {
        if (VALID_STATUSES.includes(yaml.status)) {
            next.status = yaml.status;
        }
        else {
            process.stderr.write(`[harness backlog] ${filepath}:${entry.lineNumber} status=${JSON.stringify(yaml.status)} is not one of ${JSON.stringify(VALID_STATUSES)}; falling back to "pending".\n`);
            next.status = "pending";
        }
    }
...
export function parseBacklog(filepath) {
    ...
    const entries = [];
    const seenIds = new Map(); // id -> first lineNumber
    let i = 0;
    while (i < lines.length) {
        ...
        if (fenceMatch !== null) {
            // Treat any fenced code block as opaque — only YAML-tagged fences
            // contribute to entry metadata, but every fence (yaml / ts / bash /
            // none / etc.) must be scanned to its close so its body cannot
            // contain a heading-shaped line that the parser would mistake for
            // a new entry.
            const isYamlFence = typeof fenceMatch[1] === "string" &&
            ...
            while (bodyEnd < lines.length) {
                ...
                // Recovery for an unterminated fence: scanning to EOF would
                // silently swallow every following entry. Stop at the next
                // heading-shaped line and resume from it (after warning).
            ...
            if (isYamlFence) {
                ...
                if (malformed) {
                    process.stderr.write(`[harness backlog] ${filepath}:${lineNumber} YAML metadata block is malformed (one or more lines did not match key:value); falling back to heading defaults for unparsable lines.\n`);
                }
                entry = applyYamlOverrides(entry, fields, filepath);
            }
            if (!closedProperly) {
                // Unterminated fence — warn so the operator can fix the markdown.
                // The YAML branch already parsed the partial body so its metadata
                // is not entirely lost.
                process.stderr.write(`[harness backlog] ${filepath}:${lineNumber} ${isYamlFence ? "YAML fence opened with ```yaml" : "fenced block"} but no closing \`\`\` before ${nextHeadingHit ? `next heading at line ${bodyEnd + 1}` : "end of file"}; treating fence as unclosed and recovering.\n`);
            }
            if (nextHeadingHit) {
                // Resume *at* the heading so it is parsed in the next iteration
                // (do not skip past it). Same behavior for YAML and non-YAML.
                i = bodyEnd;
            }
            else {
                // Resume after the closing fence (or end-of-file when unclosed).
                // Same behavior for YAML and non-YAML — fence is always opaque.
                i = bodyEnd + 1;
            }
        }
        else {
        ...
        if (firstSeen !== undefined) {
            process.stderr.write(`[harness backlog] ${filepath}:${entry.lineNumber} duplicate id "${entry.id}" (first seen at line ${firstSeen}); both entries retained for dispatcher inspection.\n`);
        }
        else {
