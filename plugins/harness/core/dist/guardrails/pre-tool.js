...
import { predictBudgetImpact, normaliseProjectRelative, } from "../context-audit/index.js";
...
export async function evaluatePreTool(input) {
...
async function augmentWithContextAudit(prev, ctx, input) {
    ...
    if (input.tool_name !== "Write")
        return prev;
    const rawFilePath = extractStringField(input.tool_input, "file_path");
    const content = extractStringField(input.tool_input, "content");
    if (rawFilePath === undefined || content === undefined)
        return prev;
    // Normalise `tool_input.file_path` to a project-relative POSIX path before
    ...
    const normalisedFilePath = normaliseProjectRelative(rawFilePath, ctx.projectRoot);
    // Paths outside the project root are not auto-load targets — short-circuit.
    if (normalisedFilePath === null)
        return prev;
    const filePath = normalisedFilePath;
    let prediction;
    try {
        ...
        process.stderr.write(`[harness pre-tool] context-audit error: ${err instanceof Error ? err.message : String(err)}\n`);
        return prev;
    }
    if (!prediction.targetIsAutoLoad)
        return prev;
    if (!prediction.wouldExceed)
        return prev;
    ...
    const merged = prev.additionalContext !== undefined && prev.additionalContext.length > 0
...
function extractStringField(input, key) {
    const v = input[key];
