...
function parseAssistant(slug, obj, timestamp) {
...
export function readSessionLog(slug, logDir) {
    const dir = logDir ?? process.env.CLAUDE_ONESHOT_LOG_DIR ?? "/tmp";
...
function readGitBranch(worktreePath) {
    if (!existsSync(worktreePath))
        return null;
    const r = spawnSync("git", ["rev-parse", "--abbrev-ref", "HEAD"], { cwd: worktreePath, encoding: "utf-8" });
    if (r.status !== 0)
        return null;
    const out = (r.stdout ?? "").trim();
    return out || null;
...
function terminalStatusFromPhase(event) {
    ...
    if (/SHIP/i.test(haystack))
        return "ship";
    if (/FIX_FIRST/i.test(haystack))
        return "error";
    if (/actionable\s*=\s*0/i.test(haystack))
        return "actionable=0";
...
export function renderIdlePaneTitle(summary) {
    const idle = summary.idle;
    if (!idle || idle.severity === "fresh")
        return summary.slug;
...
export function planIdlePaneLabels(summaries, opts) {
...
export function renderDashboard(summaries) {
    ...
    const header = "| slug | branch | phase | last commit | status |\n| --- | --- | --- | --- | --- |";
