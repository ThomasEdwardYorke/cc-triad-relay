...
export const VALID_PROFILES = ["chill", "assertive", "strict"] as const;
...
const UPSTREAM_PROFILES: readonly CoderabbitProfile[] = ["chill", "assertive"];

function isValidProfile(value: string): value is CoderabbitProfile {
  return (VALID_PROFILES as readonly string[]).includes(value);
}

function isUpstreamProfile(value: string): value is "chill" | "assertive" {
  return (UPSTREAM_PROFILES as readonly string[]).includes(value);
}
...
export function resolveProfile(input: ProfileResolverInput): ProfileResolution {
  ...
  if (cliRaw.length > 0) {
    if (isValidProfile(cliRaw)) {
      if (cliRaw === "strict" && !allowStrict) {
        warnings.push(
          `cli profile='strict' is not allowed in this context (allowStrict=false); falling through.`,
        );
      } else {
      ...
      warnings.push(
        `cli profile='${cliRaw}' is invalid (must be one of ${VALID_PROFILES.join("/")}); falling through.`,
      );
    }
  }
  ...
  if (envRaw.length > 0) {
    if (isValidProfile(envRaw)) {
      if (envRaw === "strict" && !allowStrict) {
        warnings.push(
          `env HARNESS_CR_PROFILE='strict' is not allowed in this context (allowStrict=false); falling through.`,
        );
      } else {
      ...
      warnings.push(
        `env HARNESS_CR_PROFILE='${envRaw}' is invalid (must be one of ${VALID_PROFILES.join("/")}); falling through.`,
      );
    }
  }
  ...
  if (input.harnessConfigProfile !== undefined) {
    ...
    if (cfg === "strict" && !allowStrict) {
      warnings.push(
        `harness.config.json profile='strict' is not allowed in this context (allowStrict=false); falling through.`,
      );
    } else {
  ...
  if (yamlRaw.length > 0) {
    ...
    warnings.push(
      `.coderabbit.yaml profile='${yamlRaw}' is outside CodeRabbit upstream allowlist (chill/assertive); falling through to default.`,
    );
  }
