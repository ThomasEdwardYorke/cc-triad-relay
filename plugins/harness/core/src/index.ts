...
function extractBoolean(obj: Record<string, unknown>, key: string): boolean | undefined {
  const val = obj[key];
...
export async function route(
  ...
  switch (hookType) {
    case "pre-tool": {
      ...
      return await evaluatePreTool(input as HookInput);
    }
    case "post-tool": {
 ...
 * @internal
 */
export function sanitizeSafeFallbackReason(raw: string): string {
  ...
  return (
    codepoints.slice(0, MAX_SAFE_FALLBACK_REASON_CHARS).join("") +
    "…[truncated]"
  );
}
...
async function main(): Promise<void> {
  ...
  if (hookType === "permission" && result.systemMessage !== undefined) {
    ...
    //   - stop → "Stop" (decision omit、公式 spec で `decision: "block"` のみ許容)
    //   - subagent-stop → "SubagentStop" (decision omit、Stop と同 semantics)
    //   - pre-compact → "PreCompact" (decision omit、Stop と同 semantics)
    // content-integrity invariant (see __tests__/content-integrity.test.ts):
    // `hookEventName` 識別子から 200 chars 以内に各公式 event 名リテラルが
    // 並ぶこと。inline lookup table で 8 branch (将来 9+ も) を表現する。
    ...
    if (result.decision === "ask" || result.decision === "deny") {
      ...
      const out: Record<string, unknown> = { hookSpecificOutput: hso };
